...
for shp in shp_list:
    # Use os.path to handle paths safely instead of .split('.')
    ...
    l = len(sf)
    print(f"Number of records in shapefile: {l}")

    for c in range(l):
        try:
            shaperec = sf.record(c)
            # Accessing the name - adjust key if 'NAMELSAD' doesn't exist
            city_name = shaperec[1] if 'NAMELSAD' not in shaperec else shaperec['NAMELSAD']
            print(f"  -> Record {c}: {city_name}")

            # Get geometry
            shape = sf.shape(c)
            stArea = Polygon(shape.points)
            
            # Check if geometry is valid
            if stArea.is_empty:
                print("     Warning: Geometry is empty.")
                continue
                
            tileList = []
            # Calculate range
            ranges = getTileRange(stArea, 18)
            x_range, y_range = ranges[0], ranges[1]
            
            print(f"     Tile bounds: X({x_range}), Y({y_range})")

            for y in range(y_range[0], y_range[1] + 1):
                for x in range(x_range[0], x_range[1] + 1):
                    if (doesTileIntersects(x, y, 18, stArea)):
                        tileList.append((18, y, x))
            
            if tileList:
                y_list = [x[1] for x in tileList]
                x_list = [x[2] for x in tileList]
                pd_dict = pd.DataFrame({'y_tile': y_list, 'x_tile': x_list})
                output_path = os.path.join('tilefile_scd', f"{city_name}.csv")
                pd_dict.to_csv(output_path, index=False)
                print(f"     Success! Saved {len(tileList)} tiles to {output_path}")
            else:
                print("     No tiles found intersecting this geometry.")
                
        except Exception as e:
            print(f"  -> Error processing record {c}: {e}")
# for shp in shp_list:
#     print(shp.split('.')[0])
...
#         print(f"Checking record: {shaperec['NAMELSAD']}") # DEBUG LINE 1
...
#             print(f"Bounds: {stArea.bounds}") # DEBUG LINE 2
#             print(shaperec['NAMELSAD'])
